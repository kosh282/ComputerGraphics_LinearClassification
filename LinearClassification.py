...
if __name__ == "__main__":
    ...
    # [추가된 파트] 혼동 행렬(Confusion Matrix) 계산 및 시각화
    ...
    # 1) 텍스트 기반 혼동 행렬 및 분류 리포트 출력 (콘솔 확인용)
    cm = confusion_matrix(y_test, test_pred)
    print("\n--- [텍스트 혼동 행렬 (Confusion Matrix)] ---")
    print(cm)
    
    print("\n--- [상세 분류 리포트 (Classification Report)] ---")
    # 각 클래스별 정밀도(Precision), 재현율(Recall), F1-score 등을 보여줍니다.
    print(classification_report(y_test, test_pred, target_names=classes))

    # 2) 시각적인 히트맵(Heatmap) 그래프 그리기 (리포트/제출용)
    plt.figure(figsize=(10, 8))
    
    # sns.heatmap을 사용해 혼동 행렬을 색상 맵으로 표현합니다.
    sns.heatmap(cm, annot=True, fmt='d', cmap='Blues',
                xticklabels=classes, yticklabels=classes)
    
    plt.title('CIFAR-10 Linear Classifier Confusion Matrix', fontsize=14)
    ...
    # 그래프 창 띄우기 (또는 plt.savefig('confusion_matrix.png')로 파일 저장 가능)
    plt.show()